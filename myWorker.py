    ...
    while True:
        for s in STREAMS[stream_id]['ins']:
            ...
            for cap  in caption_tmp:
                ...
                caption_text = get_caption(cap['content'])  # drawtext -> textfile
                cur_map = g1.next()
                if cap['direction'] == '0':
                    ...
                    filter_cmd += '[%s] drawtext=textfile=%s: fontfile=%s/%s: fontsize=%s: fontcolor=%s: shadowx=%s: shadowy=%s: shadowcolor=%s: borderw=%s: bordercolor=%s: x=%s: y=%s [%s]; ' % (pre_map, caption_text, FONT_DIR, fontfile, cap['fontSize'], cap['fontColor'], cap['shadowX'], cap['shadowY'], cap['shadowColor'], cap['borderW'], cap['borderColor'], cap['positionX'], cap['positionY'], cur_map)   # drawtext -> textfile
                    ...
                    filter_cmd += '[%s] drawtext=textfile=%s: fontfile=%s/%s: fontsize=%s: fontcolor=%s: shadowx=%s: shadowy=%s: shadowcolor=%s: borderw=%s: bordercolor=%s: x=&if(gte(t,1),(main_w-mod(t*%s,main_w+text_w)),NAN)&: y=%s [%s]; ' % (pre_map, caption_text,  FONT_DIR, fontfile, cap['fontSize'], cap['fontColor'], cap['shadowX'], cap['shadowY'], cap['shadowColor'], cap['borderW'], cap['borderColor'], cap['step'], cap['positionY'], cur_map)  #drawtext -> textfile
                else:
                    # filter_cmd += '[%s] drawtext=text=%s: fontfile=%s/%s: fontsize=%s: fontcolor=%s: shadowx=%s: shadowy=%s: shadowcolor=%s: borderw=%s: bordercolor=%s: x=%s: y=%s [%s]; ' % (pre_map, cap['content'], FONT_DIR, fontfile, cap['fontSize'], cap['fontColor'], cap['shadowX'], cap['shadowY'], cap['shadowColor'], cap['borderW'], cap['borderColor'], cap['positionX'], cap['positionY'], cur_map)
                    filter_cmd += '[%s] drawtext=textfile=%s: fontfile=%s/%s: fontsize=%s: fontcolor=%s: shadowx=%s: shadowy=%s: shadowcolor=%s: borderw=%s: bordercolor=%s: x=%s: y=%s [%s]; ' % (pre_map, cap['content'], FONT_DIR, fontfile, cap['fontSize'], caption_text, cap['shadowX'], cap['shadowY'], cap['shadowColor'], cap['borderW'], cap['borderColor'], cap['positionX'], cap['positionY'], cur_map) # drawtext -> textfile
                pre_map = cur_map
